...
namespace CarSalesmenTests
{
    ...
    public class VehicleBuyer
    {
        public IEnumerable<VehicleReceipt> BuyVehicles(IEnumerable<VehicleRequest> vehiclesToBuy)
        {
            ...
            var mattsMotors = new MattsMotors();
            JimmysAutomobiles jimmysAutomobiles = new JimmysAutomobiles();

            IEnumerable<ICarDealer> dealers = new List<ICarDealer>() { new JudsonsDealer(judsonsJallopies), new MattDealer(mattsMotors), new JimmysDealer(jimmysAutomobiles) }; 

            return vehiclesToBuy.Select(request => BuyVehicle(request,dealers)).ToList();

        }

        private VehicleReceipt BuyVehicle(VehicleRequest vehicleRequest, IEnumerable<ICarDealer> carDealers)
        {
            var quoteResults = carDealers.Select(s => s.GetPurchaseTokenForVehicleRequest(vehicleRequest));
            var goodQuotes = quoteResults.Where(quoteResult => quoteResult.IsSuccess).Select(quoteResult => quoteResult.Value);

            var bestBuy = goodQuotes.Select(quote => quote.PurchaseVehicle()).OrderBy(receipt => receipt.Price.Amount).FirstOrDefault();
            return bestBuy;
        }
